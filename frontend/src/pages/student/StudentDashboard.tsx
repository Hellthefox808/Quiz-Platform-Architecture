    ...
    <div className="space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-indigo-900/60 via-slate-900 to-slate-900 border border-indigo-500/20 p-8 shadow-2xl">
        <div className="relative z-10 max-w-2xl">
          ...
          <p className="mt-2 text-slate-300 text-sm leading-relaxed">
            Browse published assessments across cybersecurity, cloud architecture, database systems, and algorithms.
          </p>
          <div className="mt-6 flex flex-wrap gap-3">
            <button
              onClick={() => onNavigate('catalog')}
              className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-semibold flex items-center gap-2 shadow-lg shadow-indigo-600/30 transition cursor-pointer"
            >
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-sm font-semibold border border-slate-700 transition flex items-center gap-2 cursor-pointer"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-400 uppercase">Total Attempts</span>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-400 uppercase">Pass Rate</span>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-400 uppercase">Average Score</span>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-400 uppercase">Total Time</span>
          ...
          <div className="mt-4">
            <span className="text-3xl font-bold text-white tracking-tight">
              {Math.round((analytics?.total_time_spent_seconds || 0) / 60)} <span className="text-base font-normal text-slate-400">min</span>
            </span>
          </div>
          <div className="mt-2 text-xs text-slate-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-sm">
          <h2 className="text-base font-bold text-white mb-4">Category Proficiency</h2>
          {analytics?.category_breakdown.length === 0 ? (
            <div className="text-sm text-slate-500 text-center py-8">
              Take your first quiz to see category breakdown.
            </div>
          ) : (
              ...
              {analytics?.category_breakdown.map((cat) => (
                <div key={cat.category} className="space-y-1.5">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-semibold text-indigo-400">{cat.avg_percentage}%</span>
                  </div>
        ...
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base font-bold text-white">Recent Attempts</h2>
            <button
          ...
          {recentAttempts.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-slate-400">You haven't attempted any quizzes yet.</p>
              <button
                onClick={() => onNavigate('catalog')}
                ...
                <div key={att.id} className="py-3.5 flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <div className="text-sm font-bold text-white">{att.percentage}%</div>
                      <span
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 text-xs bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg transition border border-slate-700"
                    >
                      Result
                    </button>
                  </div>
